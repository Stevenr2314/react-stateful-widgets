...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const {name} = programmers.filter(elem => elem.id === featuredProgrammer)[0]
    return name
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {featuredProgrammer ? setFeaturedProgrammer(null) : setFeaturedProgrammer(dev.id)}}>Toggle Feature</button>
            </div>
          )
        }
      </div>
